...
# 5. Multiprocessing settings
# Use 100 lines per task chunk. Good for large files.
CHUNK_SIZE = 100
# ---------------------

#####################################################################
# WORKER FUNCTION (This is what runs in parallel)
#####################################################################
def process_line(line, png_output_dir, npy_output_dir, padding, stroke_width, png_size, normalize_dim):
    """
    Processes a single line (one drawing) from the .ndjson file.
    ...
    try:
        ...
        # --- Path A: Generate PNG ---
        svg_data_string = ndjson_to_svg_data(strokes, word, padding, stroke_width, png_size)
        ...
        print(f"  Error processing line (key: {data.get('key_id', 'N/A')}): {e}")
...
def ndjson_to_svg_data(strokes, word, padding, stroke_width, png_size):
    all_x, all_y = [], []
    svg_paths    = []
    ...
    svg_content = (
            ...
            f'  <title>{word}</title>\n'
            f'  <rect width="100%" height="100%" fill="white"></rect>\n'
            + '\n'.join(svg_paths)
            + '\n</svg>'
        )
    return svg_content
...
#####################################################################
# NPY (SEQUENCE) GENERATION FUNCTIONS (Unchanged)
#####################################################################
...
#####################################################################
# MAIN SCRIPT EXECUTION (Rewritten for multiprocessing)
#####################################################################
def main():
    ...
    for file_name in FILES_TO_PROCESS:
        ...
        # This pre-fills all the arguments for the worker function
        # that don't change for this file.
        worker_func = partial(
            process_line,
        ...
        try:
            with Pool() as pool:
                # Get the total line count for the progress bar
                with open(ndjson_file_path, 'r') as f:
                    for _ in f:
                        line_count += 1
                
                # Open the file again to start processing
                with open(ndjson_file_path, 'r') as f:
                    # pool.imap_unordered processes lines in parallel
                    # chunksize=100 feeds 100 lines at a time to each worker (efficient)
                    # tqdm wraps this to create a live progress bar
                    results = pool.imap_unordered(worker_func, f, chunksize=CHUNK_SIZE)
                    
                    for success in tqdm.tqdm(results, total=line_count, desc=f"  Processing"):
                        if success:
            ...
            print(f"  A critical error occurred while processing {file_name}: {e}")
...
if __name__ == "__main__":
    # This is required for multiprocessing to work correctly on
    # some systems (like Windows)
    main()
